...
- To use the public API you do not need anything special, just run any public method from either general or market endpoints.

- To use the private API you first have to add a new entry in your user's path called "BINANCE_API_KEY" and "BINANCE_API_SECRET" which must contain the API key/ secret created on Binance.com

  - note: The secret is also needed as Binance needs all private API interactions to be hmac signed.

- All the available wrapper functions can be found in _EXAMPLE.py.

- Official API documentation: https://www.binance.com/en/binance-api
...
### Installation
```
git clone https://github.com/ageorge95/pyBinance pyBinance
cd pyBinance
sh install.sh
```
